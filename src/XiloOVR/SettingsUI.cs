...
public sealed class SettingsUI : IDisposable
{
    ...
    private void OpenChannelKeyboard()
    {
        if (_keyboardOpen)
            return;
        var error = OpenVR.Overlay.ShowKeyboardForOverlay(
            ...
            0, "Twitch channel name (empty = chat off)", 64, _config.TwitchChannel, 0);
    ...
    private void OnKeyboardDone()
    {
        ...
        _config.TwitchChannel = buffer.ToString().Trim().TrimStart('#').ToLowerInvariant();
        _applyAndSave();
        _dirty = true;
    }
    ...
    private void Render()
    {
        ...
        using (var g = Graphics.FromImage(bitmap))
        {
            ...
            Button(new Rectangle(rightX + colWidth - 92, y2 + 4, 92, ButtonSize), "Edit", OpenChannelKeyboard);
            y2 += RowHeight;
            ...
            foreach (var planned in new[]
                     {
                         "Twitch login + send chat replies from VR   (v0.5)",
                         "Follow / sub / raid alerts on the panel   (v0.5)",
                         "YouTube chat merged into the same feed   (v0.6)",
                     })
            {
    ...
    private void UploadThumbnail()
    {
        ...
        using (var g = Graphics.FromImage(bitmap))
        {
            ...
            using var pen = new Pen(Accent, 6);
